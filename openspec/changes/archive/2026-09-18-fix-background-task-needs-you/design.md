...
**Decision: compact the array in the hook to `{id, type, status, description}`, forwarded on `Stop` and `SubagentStop`; only LIVE (`running` or `pending`) entries of an AGENT-LIKE type (`subagent`, `workflow`, `cloud session`) count.** `pending` is queued work that flips to running with no hook; `shell`/`monitor`/housekeeping entries are excluded because they may never exit, and `teammate` because an idle teammate stays `running` (idleness is a separate, unforwarded flag) and is evicted without a hook. `SubagentStop` also forwards `agentId` so the interrupt path can subtract finished agents (`outstandingBackgroundTasks`). A trailing idle-prompt `Notification` inherits the running state of the Stop that ended the parent's turn, looking back past SubagentStops and tool traffic (a background subagent's tool hooks fire in the same process with the same pane id, so they land in the parent's ring) and subtracting agents that have since stopped.
...
- [The Stop that listed the background agent is evicted from the 500-event ring by a long subagent's tool traffic, then the user presses Esc mid-tool] → the synthetic turn-end carries no list and the row reads Needs you until the subagent's next tool event; accepted (the user did press Esc; a per-pane sticky latch could close it later).
- [User prompts while a background agent is out, presses Esc before any tool runs, then idles ~60 s] → the ~5 s sink re-seed drops the list-carrying synthetic Stop once the idle Notification is persisted, and the look-back stops at the `UserPromptSubmit`, so the row reads Needs you until the agent's next hook; accepted (strictly better than before, where it flipped immediately; closing it needs the seed rule to keep a listed synthetic Stop behind Notification/SubagentStop-only successors).
