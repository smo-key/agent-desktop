...
- [x] 6.7 BUGFIX (caught in 5.3 live verify — no LIVE subagents ever showed): `scan_parent_tool_state` only recognized `tool_use` blocks named `Task`, but this app's harness names the subagent-spawning tool `Agent`. So a live subagent's `tool_use` was never matched, `running` was always false, every standalone subagent was marked `done`, and the live-only filter (7.2) hid them the entire time they were alive. Fix: match BOTH `Agent` and `Task` tool_use names. Regression test: `standalone_running_detected_for_agent_tool`.
