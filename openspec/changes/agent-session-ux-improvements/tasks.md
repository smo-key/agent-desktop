...
- [ ] 21.2 Re-run `npm run coverage` (scenario coverage gate) and ensure every new scenario (footer popovers, review-action tooltip, push/commit popovers, open-PRs popover + draft exclusion, per-branch PR bubble, card model, footer model/effort pills, snapshot field shape, freshly-launched coordinator Waiting, agent card hides context when unknown, footer usage-tooltip reset time) has a matching test.
