...
- [x] 21.1 Re-run `npm run check` (svelte-check) and `npm run test` (vitest), and `cargo test` (manifest `src-tauri/Cargo.toml`) for the new/changed Rust (`usage` snapshot fields, `pr` list, `git` commits_to_push) and the statusline wrapper; fix any failures introduced by the new work. (`events::tests` SUN_LEN failures remain pre-existing/environmental — `events.rs` is untouched.)
- [x] 21.2 Re-run `npm run coverage` (scenario coverage gate) and ensure every new scenario (footer popovers, review-action tooltip, push/commit popovers, open-PRs popover + draft exclusion, per-branch PR bubble, card model, footer model/effort pills, snapshot field shape, freshly-launched coordinator Waiting, agent card hides context when unknown, footer usage-tooltip reset time) has a matching test.
- [x] 21.3 Manually verify the headline flows in the running app. (Verified interactively by the developer across the session — repeated "looks good" sign-offs on the footer/popover/card flows — who then directed close-out via `/workflow-done`.) Original: coordinated compass badge + tooltip, archive a coordinator (+ label), `! sleep 999` shows In flight, a dynamic-workflow session shows In flight, last-message line incl. archived, auto-advance toggle, counters exclude archived, rename via header + menu, title refresh after a message, titles that reflect the original request in a long session, ⌘O inserts a filename. New: agent card shows model (e.g. "Opus 4.6") not cost; footer model + effort pills (effort omitted when unsupported); uncommitted tooltip says "click to review"; push pill → popover lists commits + "Push now" (tooltip "click to review"); uncommitted pill → popover lists files + "Commit now"; per-branch PR bubble shows "PR #N" / gray "PR" (create on click), separate from the open-PRs pill; open-PRs pill → popover lists PRs (active first, drafts last) with row-open + "open PRs page"; PR warning ignores drafts; popovers scroll with a pinned button and close on outside-click/Escape; popover primary-action buttons are ORANGE + consistent and clicking them closes the popover immediately. Also: a just-launched coordinator reads Waiting (Needs you) until first prompted, then quiet-stays-Working; a just-spawned agent's card shows NO context bar/% until its first snapshot lands; a footer 5h/7d bar tooltip names the absolute reset time (time-only same day, date+time otherwise).

## 22. Freshly launched coordinator reads Waiting (agent-status-derivation)

- [x] 22.1 Add `everPrompted` to `EventActivity` (events.ts), set true once a `UserPromptSubmit` has been observed; default false in `EMPTY` and treat absent as false.
- [x] 22.2 In `rowFor` (roster.ts), make a LIVE coordinator that has never been prompted (`!everPrompted`) read `waiting` — an exception to the quiet-stays-`working` suppression, which still applies once it has started a turn. The pending-question / `request_user_input` cases keep forcing `waiting`.
- [x] 22.3 Tests: `deriveEventActivity` tracks `everPrompted` across the first prompt + later turns; a freshly launched coordinator (no events / `everPrompted:false`) is `waiting` + needsAttention; an engaged-but-quiet coordinator stays `working`; the terminal-busy coordinator test uses an engaged coordinator.

## 23. Agent card hides context until a size is known (agent-roster-display)

- [x] 23.1 Add a PURE `showContext(row)` to roster.ts — live (not closed/preview/paused) AND `contextPct !== null`.
- [x] 23.2 In `Inbox.svelte`, gate the context mini-bar + percent on `showContext(r)` (replacing the old `showMeta` that showed a placeholder dash when unknown); drop the now-unused `showMeta`.
- [x] 23.3 Tests for `showContext`: shown for a live known percent (incl. 0); hidden when null; hidden for archived/previewed/paused even with a value.

## 24. Footer usage tooltips show when each window resets (usage-dashboard)

- [x] 24.1 Add PURE `resetClause` + `usageLimitTooltip` (with an injectable `ClockFormat`) to `timeRemaining.ts`: absolute local reset time — time-only on the same calendar day, date + time otherwise; omit the reset clause when unknown.
- [x] 24.2 In `LimitBars.svelte`, build each bar's StatusBar tooltip with `usageLimitTooltip(name, usedPct, resetsAt, now)`.
- [x] 24.3 Tests: `resetClause` same-day vs different-day vs unknown (stub formatter, TZ pinned); `usageLimitTooltip` composition incl. unknown reset + unknown percent.

## 25. Remove dead helpers superseded during this change (no behavior change)

- [x] 25.1 Drop `prButtonDisabled` (prActions.ts) — the per-branch PR bubble (group 20) hides/shows itself; there is no longer a base-branch "disabled" rule. The footer-actions spec already describes hide-not-disable, so no spec change.
- [x] 25.2 Drop `onCommitButtonClick` (prActions.ts) — superseded by the commit popover's `spawnCommitFromPopover` (group 16). `spawnCommit` (the shared spawn) stays.
