...
- **THEN** the session stays `working`; a `Notification` behind a `Stop` with no such outstanding task, or after a new `UserPromptSubmit`, still yields `waiting`
