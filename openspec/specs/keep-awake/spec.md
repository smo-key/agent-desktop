# keep-awake Specification

## Purpose
Keep the computer from idle-sleeping while agents work (or while the app is open) so their network connections survive: the Settings preference, its persistence, the mode + roster resolution rule, and the best-effort platform sleep inhibitor that never outlives the app.
## Requirements
