# dialog-autofocus Specification

## Purpose

Guarantee that opening any dialog places keyboard focus on a meaningful first
control, so the keyboard can drive the dialog immediately — without a click or a
Tab to reach in. Defines the shared `use:autofocus` Svelte action that implements
this and the per-dialog focus targets (chosen for safety, not just DOM order).
