...
- **AND** the panel shows each project's total agent count plus its waiting/working breakdown

#### Scenario: Waiting and working counts are shown independently
...
- **THEN** the rail badge shows the waiting count (attention-colored); a project that is only working shows the working count
