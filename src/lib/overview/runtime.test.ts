...
describe('runtime registry', () => {
  it('records output activity and maps to working/waiting via deriveStatus', () => {
    const now = 1_000_000;
    noteOutput('p1', now);
    expect(getRuntime('p1')).toEqual({ lastOutputAt: now, exited: false, exitCode: null, spawnedAt: expect.any(Number) });
  ...
  it('runtimeMap returns detached shallow copies', () => {
    ...
    noteOutput('p1', now);
    const m = runtimeMap();
    expect(m.p1).toEqual({ lastOutputAt: now, exited: false, exitCode: null, spawnedAt: expect.any(Number) });
